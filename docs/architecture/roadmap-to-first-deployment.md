...
| `docs/architecture/adr-0005-osm-free-stack.md` | **Amends ADR-0003.** OpenStreetMap leaves the stack: BKG basemap.de (CC BY 4.0) for the map, BKG CLC5 classes 111/112 for settlement geometry, so ODbL share-alike never reaches `criterion_value`. Closes **U7**. |
...
| U7 | Per-source licensing | **Closed (2026-09-19) — ADR-0005.** All five datasets' terms are read at a primary source. BKG CLC5-2018 (`dl-de/by-2-0`), DWD CDC (CC BY 4.0) and BKG VG25 (CC BY 4.0) are Confirmed and fetched. The ODbL question is decided: **OpenStreetMap leaves the stack** — basemap.de for the map, CLC5 classes 111/112 for settlement geometry — so no share-alike term touches sela's database and §4.6 never attaches. `osm-geofabrik` is **withdrawn**, not unconfirmed. BfN's licence (GeoNutzV) is cleared but `geodienste.bfn.de` still returns 403, which is an access problem rather than a licensing one and is tracked as such. Attribution is now structurally required (`source.attribution NOT NULL`), closing §7 condition 4 |
